...
function counters(){for(const g of ['hangman','blackjack','daily','tictactoe','fade','dots']) if($('#count-'+g)) $('#count-'+g).textContent=access.unlimited?'Unlimited play':access[g]+' free '+(access[g]===1?'play':'plays')+' left';$('#account-label').textContent=access.unlimited?'Your forever pass is active.':'Just you. No sign-up.';$$('[data-unlock]').forEach(b=>{if(access.unlimited)b.textContent='Your unlimited pass ↗';});}
